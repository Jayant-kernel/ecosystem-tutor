...
export interface RockLayerHandle {
  setProgress: (level: number, parallaxX?: number, parallaxY?: number) => void;
}
 ...
 * a thin cool rim make them read as rock under violet light. The two distant
 * pieces get a deliberately faint ion tail; no downloaded image or WebGL is
 * needed.
 */
const ROCKS: RockConfig[] = [
  { id: 'fore-left', seed: 101, left: 0.02, top: 0.56, size: 340, compactSize: 178, compactLeft: -0.11, compactTop: 0.68, fromXvw: -14, fromYvh: 8, fromRot: -8, opacity: 0.92, blur: 0, driftSeconds: 22, parallaxDepth: 1 },
  { id: 'mid-right', seed: 202, left: 0.78, top: 0.28, size: 196, compactSize: 108, fromXvw: 12, fromYvh: -5, fromRot: 8, opacity: 0.88, blur: 0, driftSeconds: 26, parallaxDepth: 0.55, tail: true },
  { id: 'mid-left', seed: 303, left: 0.13, top: 0.16, size: 142, compactSize: 86, compactTop: 0.1, fromXvw: -9, fromYvh: -6, fromRot: -6, opacity: 0.82, blur: 0.5, driftSeconds: 24, parallaxDepth: 0.5, tail: true },
  { id: 'bg-right', seed: 404, left: 0.7, top: 0.64, size: 78, fromXvw: 7, fromYvh: 5, fromRot: 5, opacity: 0.44, blur: 3, driftSeconds: 30, parallaxDepth: 0.3, tail: true },
  { id: 'bg-left', seed: 505, left: 0.3, top: 0.72, size: 60, fromXvw: -6, fromYvh: 6, fromRot: -5, opacity: 0.36, blur: 5, driftSeconds: 32, parallaxDepth: 0.28 },
];
...
const RockLayer = forwardRef<RockLayerHandle>(function RockLayer(_, ref) {
  ...
  useImperativeHandle(ref, () => ({
    setProgress(level: number, parallaxX = 0, parallaxY = 0) {
      ...
      ROCKS.forEach((rock, i) => {
        ...
        const parallax = parallaxEls.current[i];
        if (parallax) {
          parallax.style.transform =
            `translate(${(parallaxX * rock.parallaxDepth).toFixed(2)}px, ${(parallaxY * rock.parallaxDepth).toFixed(2)}px)`;
        }
      });
    },
  }), []);
    ...
    <>
      {ROCKS.map((rock, i) => {
        ...
        return (
          ...
          >
            <div ref={(el) => { parallaxEls.current[i] = el; }} className="h-full w-full">
              <div className="h-full w-full" style={{ animation: `rock-drift ${rock.driftSeconds}s ease-in-out infinite alternate` }}>
                <svg viewBox="0 0 100 100" className="h-full w-full" style={{ overflow: 'visible' }}>
                  <defs>
                    ...
                    <linearGradient id={`${gid}-tail`} x1="1" y1=".5" x2="0" y2=".5">
                      <stop offset="0%" stopColor="#c4b5fd" stopOpacity="0.38" />
                      <stop offset="26%" stopColor="#7c3aed" stopOpacity="0.14" />
                      <stop offset="100%" stopColor="#312e81" stopOpacity="0" />
                    </linearGradient>
                    <clipPath id={`${gid}-clip`}><path d={shape} /></clipPath>
                  </defs>
                  {rock.tail ? <path d="M43 42 C23 31, 2 27, -20 36 C4 42, 19 52, 43 57 Z" fill={`url(#${gid}-tail)`} /> : null}
                  ...
                  <g clipPath={`url(#${gid}-clip)`}>
                    ...
                    {craters.map((crater, craterIndex) => (
                      <g key={craterIndex} transform={`rotate(${crater.rotation} ${crater.x} ${crater.y})`} opacity={crater.opacity}>
                        <ellipse cx={crater.x} cy={crater.y} rx={crater.rx + 1.4} ry={crater.ry + 1.2} fill="#c4b5fd" opacity="0.16" />
                  ...
                  <path d={shape} fill="none" stroke="#f5f3ff" strokeOpacity="0.1" strokeWidth="0.8" />
                </svg>
              </div>
            </div>
          </div>
