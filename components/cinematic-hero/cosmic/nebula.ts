/**
 * Lightweight 2D-canvas deep-space nebula. No WebGL, no dependencies, no GLSL:
 ...
 * the hero is visible. Seeded PRNG keeps every load identical. Palette is
 * deep-space indigo with ember accents to match the landing's orange/black
 * identity.
 */
...
// Deep, dark-first palette: most blobs are near-black indigo so luminous
// regions read as light inside darkness, not as a color wash. Ember tiers
// echo the orb journey's glow.
const PALETTE: Array<{ color: string; alpha: number }> = [
  { color: '#05070f', alpha: 0.6 },
  { color: '#0a1024', alpha: 0.55 },
  { color: '#131b3d', alpha: 0.5 },
  { color: '#232f63', alpha: 0.42 },
  { color: '#3730a3', alpha: 0.3 },
  { color: '#b45309', alpha: 0.24 },
  { color: '#fdba74', alpha: 0.16 },
];
...
export class NebulaRenderer {
  ...
  constructor(private readonly canvas: HTMLCanvasElement) {
    ...
    for (let i = 0; i < BLOB_COUNT; i++) {
      // Dark deep-space base blobs first, luminous cores last (painted over).
    ...
    // One tiny warm-white heart, always central.
    this.blobs.push({
      bx: 0.5, by: 0.44, radius: 0.07,
      ax: 0.008, ay: 0.008, sx: 0.06, sy: 0.05, phase: 1.3,
      color: '#ffedd5', alpha: 0.5, stretch: 1.4, rotation: 0.6,
    });
