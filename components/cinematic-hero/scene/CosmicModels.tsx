...
import { COSMIC_REVEAL_END, COSMIC_REVEAL_START, smoothstep } from '../timeline';

// Optimized runtime assets served from public/ (never bundled): the GLB
// models stay out of the JS chunk and load lazily with the space act.
const EARTH_URL = '/cinematic-assets/earth.glb';
const EARTH_TEXTURE_URL = '/cinematic-assets/earth-diffuse.jpg';
const ASTEROIDS_URL = '/cinematic-assets/asteroids.glb';
...
/** Render Earth's diffuse map unlit in its natural colors so the satellite
 * texture stays crisp in the dark scene. A null texture (download failed)
 * falls back to flat deep blue. */
function cloneEarth(source: THREE.Object3D, diffuseMap: THREE.Texture | null): THREE.Object3D {
  ...
  earth.traverse((child) => {
    ...
    child.material = diffuseMap
      ? new THREE.MeshBasicMaterial({ map: diffuseMap, fog: true })
      : new THREE.MeshBasicMaterial({ color: '#1e3a5f', fog: true });
  });
...
/**
 * Real GLB artwork for the final space act, loaded lazily from public runtime
 * URLs (never bundled). Models load manually (not via useLoader) with
 * independent Earth/rock requests so whichever arrives first renders first.
 * Scroll changes the reveal; every asteroid also spins continuously as a
 * true 3D model, so the demand renderer stays awake while the space act is
 * visible — all inside the hero's existing canvas and render loop.
 */
export default function CosmicModels(): JSX.Element {
  ...
  useEffect(() => {
    ...
    // Earth and rocks load INDEPENDENTLY: whichever arrives first renders
    // first instead of Earth waiting on the rocks.
    gltfLoader
      .loadAsync(EARTH_URL)
      .then((earth) =>
        textureLoader
          .loadAsync(EARTH_TEXTURE_URL)
          .catch((error: unknown) => {
            // eslint-disable-next-line no-console
            console.warn('[cosmic] earth texture failed, using flat blue.', error);
            return null;
          })
          .then((texture) => {
            if (cancelled) return;
  ...
  useFrame((_, rawDelta) => {
    ...
    const level = smoothstep(COSMIC_REVEAL_START, COSMIC_REVEAL_END, filmDriver.currentT);
    ...
    if (warmLightRef.current) warmLightRef.current.intensity = 8 * level;
  ...
  return (
    <group ref={rootRef} visible={false}>
      {/* Horizon only: Earth sits low so just its top arc shows below. */}
      {earth && (
      ...
      <pointLight ref={warmLightRef} position={[-12, 4, 0]} color="#fdba74" distance={40} intensity={0} />
      ...
      <directionalLight ref={asteroidFillRef} position={[-3, 7, 5]} color="#e8edff" intensity={0} />
    </group>
  );
}
